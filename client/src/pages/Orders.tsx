  ...
  const handleUpdateWorkStatus = (status: string) => {
    if (selectedOrder) {
      const handleUpdateWorkStatus = (status: string) => {
        if (selectedOrder) {
          // Defer update
          setPendingUpdates(prev => ({ ...prev, workStatus: status }));

          // Update UI explicitly for immediate feedback (optional, but good for UX)
          setSelectedOrder({ ...selectedOrder, workStatus: status as any, ...pendingUpdates, workStatus: status });
        }
      };

      const handleUpdateDeliveryStatus = (status: string) => {
        if (selectedOrder) {
          setPendingUpdates(prev => ({ ...prev, deliveryStatus: status }));
          setSelectedOrder({ ...selectedOrder, deliveryStatus: status as any, ...pendingUpdates, deliveryStatus: status });
        }
      };
